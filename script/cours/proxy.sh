...
echo "https_proxy = http://192.168.151.254:3128/" >> /etc/wgetrc
echo "http_proxy = http://192.168.151.254:3128/" >> /etc/wgetrc
...
if command -v docker &> /dev/null; then
  ...
  echo 'Environment="HTTP_PROXY=http://192.168.151.254:3128/" "HTTPS_PROXY=http://192.168.151.254:3128/"' >> /etc/systemd/system/docker.service.d/http-proxy.conf
fi
